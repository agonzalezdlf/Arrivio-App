...
import { Search, MapPin, Clock, Check, HelpCircle } from 'lucide-react';
    ...
    return [
      {
        ...
        status: 'completed',
        time: '06:30 AM',
        location: isPickup ? (pkg.storeName || 'Merchant Center') : 'Serrano Retail Bay #4, Madrid',
      ...
      },
      {
        ...
        time: pkg.predictedArrival ? `${pkg.predictedArrival} AM` : '09:30 AM',
      ...
      },
      {
        ...
        time: (pkg.status as string) === 'delivered' || (pkg.status as string) === 'success' ? '10:15 AM' : undefined,
                    ...
                    >
                      <div className="flex items-start justify-between w-full">
                        <div>
                          ...
                          <div className="text-[13px] font-black uppercase leading-tight truncate max-w-[155px]">
                            {pkg.stopType === 'pickup' ? (pkg.storeName || 'Zara Madrid') : pkg.userId}
                          </div>
                          <div className={cn("text-[10px] font-mono mt-0.5", isCur ? "text-blue-200" : "text-slate-405")}>
          ...
          <div className="lg:col-span-8 bg-white rounded-[24px] border border-slate-200 p-6 md:p-8 shadow-sm space-y-6 text-left">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b pb-5 gap-3">
              <div>
                ...
                <h2 className="text-[18px] font-black text-slate-905 tracking-tight mt-1">Journey: {activePackage.id}</h2>
                <div className="flex items-center gap-2 mt-1 flex-wrap">
                  <span className="text-slate-700 text-xs font-black uppercase text-slate-805">{activePackage.stopType === 'pickup' ? (activePackage.storeName || 'Zara Madrid') : activePackage.userId}</span>
                ...
                return (
                  <div key={idx} className="relative">
                    <div className={cn(
                      ...
                      isComp ? "bg-blue-600 border-blue-500 text-white" : isCur ? "border-blue-500 text-blue-600 scale-105 animate-pulse" : "border-slate-200 text-slate-300"
                    )}>
                      {isComp ? <Check className="w-4 h-4 stroke-[3px]" /> : <div className={cn("w-2 h-2 rounded-full", isCur ? "bg-blue-600" : "bg-slate-200")} />}
                    </div>

                    <div className="pl-4 space-y-1">
                      <h4 className={cn("text-[14px] md:text-[15px] font-black tracking-tight", isComp ? "text-slate-800" : isCur ? "text-blue-600" : "text-slate-400")}>
                        {step.title}
                      </h4>
